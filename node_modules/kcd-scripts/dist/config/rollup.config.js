...
const {
  sizeSnapshot
} = require('rollup-plugin-size-snapshot');
...
const input = glob.sync(fromRoot(process.env.BUILD_INPUT || ifFile(`src/${format}-entry.js`, `src/${format}-entry.js`, 'src/index.js')));
const codeSplitting = input.length > 1;

if (codeSplitting && uniq(input.map(single => path.basename(single))).length !== input.length) {
  throw new Error('Filenames of code-splitted entries should be unique to get deterministic output filenames.' + `\nReceived those: ${input}.`);
}
...
const globals = parseEnv('BUILD_GLOBALS', isPreact ? Object.assign(defaultGlobals, {
  preact: 'preact'
}) : defaultGlobals);
const external = parseEnv('BUILD_EXTERNAL', isPreact ? defaultExternal.concat(['preact', 'prop-types']) : defaultExternal).filter((e, i, arry) => arry.indexOf(e) === i);
...
module.exports = {
  ...
  plugins: [isNode ? nodeBuiltIns() : null, isNode ? nodeGlobals() : null, nodeResolve({
    ...
    printInfo: false
  }) : null, minify ? terser() : null, codeSplitting && ((writes = 0) => ({
    onwrite() {
