...
const useBuiltinConfig = !args.includes('--config') && !hasFile('.prettierrc') && !hasFile('prettier.config.js') && !hasPkgProp('prettierrc');
...
const write = args.includes('--no-write') ? [] : ['--write']; // this ensures that when running format as a pre-commit hook and we get
...
const result = spawn.sync(resolveBin('prettier'), [...config, ...ignore, ...write, ...filesToApply].concat(relativeArgs), {
  stdio: 'inherit'
});
