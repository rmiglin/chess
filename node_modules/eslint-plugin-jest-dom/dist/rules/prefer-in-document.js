...
function isAntonymMatcher(matcherNode, matcherArguments) {
  return matcherNode.name === "toBeNull" || matcherNode.name === "toHaveLength" && matcherArguments[0].value === 0;
}
...
const create = context => {
    ...
    if (_queries.queries.includes(query)) {
      context.report({
        ...
        fix(fixer) {
          ...
          if (matcherNode.name === "toHaveLength" && matcherArguments[0].value === 1 && query.indexOf("All") > 0) {
  ...
  function getQueryNodeFrom(expression) {
    return expression.type === "TSAsExpression" ? getQueryNodeFrom(expression.expression) : expression.type === "AwaitExpression" ? getQueryNodeFrom(expression.argument) : expression.callee;
  }
